  ...
  return (
    <footer className="w-full bg-black shadow-2xl border-t border-white/8 text-gray-300 px-6 md:px-12 py-14">
      <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-5 gap-10 ">
        ...
        {/* Products */}
        <div>
          <h3 className="text-white font-semibold text-lg mb-4 md:mt-2 lg:mt-2 tracking-widest font-Roboto">
            Products
          </h3>
          ...
          <ul className="space-y-2 text-sm">
            {Company.map((item, i) => (
              <Link
                to={item.link}
                key={i}
                className="hover:text-blue-400 cursor-pointer duration-200 flex flex-col tracking-widest font-Roboto"
              >
                {item.label}
              </Link>
            ))}
