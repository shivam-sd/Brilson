...
const ManageNFCCard = () => {
  ...
  // ✅ NFC CARD COLORS - Dynamic (User can change)
  ...
  const downloadCard = async (card) => {
    try {
      ...
      loadingDiv.innerHTML = `
        ...
        <p class="text-white text-lg">Generating NFC Card...</p>
      ...
      link.href = url;
      link.download = `brilson-nfc-card-${card.activationCode}.png`;
      ...
      toast.success("NFC Card downloaded successfully!");
  ...
  // ✅ SINGLE VISITING CARD DOWNLOAD - Using Fixed Colors
  const downloadVisitingCard = async (card) => {
    try {
      const loadingDiv = document.createElement('div');
      loadingDiv.style.cssText = `
        position: fixed;
      ...
      loadingDiv.innerHTML = `
        ...
        <p class="text-white text-lg">Generating Visiting Card...</p>
        <p class="text-gray-400 text-sm">${card.activationCode}</p>
      `;
      document.body.appendChild(loadingDiv);

      // ✅ Using Fixed Visiting Card Colors
      const params = new URLSearchParams({
        cardBgColor: VISITING_CARD_COLORS.bgColor,
        cardTextColor: VISITING_CARD_COLORS.textColor,
        qrDotsColor: VISITING_CARD_COLORS.qrDotsColor,
        qrBgColor: VISITING_CARD_COLORS.qrBgColor,
        accentColor: VISITING_CARD_COLORS.accentColor
      });

      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/api/visiting-cards/${card._id}/download?${params.toString()}`,
        {
          responseType: 'blob',
          headers: { Authorization: `Bearer ${localStorage.getItem("adminToken")}` },
          timeout: 60000
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `brilson-visiting-card-${card.activationCode}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      setTimeout(() => URL.revokeObjectURL(url), 5000);
  ...
  // ✅ BULK NFC CARDS DOWNLOAD
  const downloadBulkNFCCards = async () => {
    if (cards.length === 0) {
      toast.error("No cards available on this page to download");
      return;
    }
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `brilson-nfc-cards-page-${currentPage}.zip`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      setTimeout(() => URL.revokeObjectURL(url), 5000);
      ...
      if (failedCards.length > 0) {
        toast.success(`Downloaded ${successfulCards.length} NFC cards, ${failedCards.length} failed`);
      } else {
        toast.success(`All ${successfulCards.length} NFC cards downloaded successfully!`);
      }
  ...
  // ✅ BULK VISITING CARDS DOWNLOAD - Using Fixed Colors
  const downloadBulkVisitingCards = async () => {
    if (cards.length === 0) {
      toast.error("No cards available on this page to download");
      return;
    }
    ...
    loadingDiv.innerHTML = `
      ...
      <p class="text-white text-xl font-semibold">Downloading ${cards.length} Visiting Cards...</p>
      <p class="text-gray-400 text-sm">Page ${currentPage} of ${totalPages}</p>
      <div class="w-64 bg-gray-700 rounded-full h-2.5">
    ...
    try {
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        try {
          ...
          if (response.data && response.data.size > 0) {
            const fileName = `brilson-visiting-card-${card.activationCode}.png`;
            zip.file(fileName, response.data);
            successfulCards.push(card);
          } else {
            failedCards.push(card);
          ...
          console.error(`Failed to download visiting card ${card.activationCode}:`, error);
          failedCards.push(card);
          updateFailed(failedCards.length);
        }
      }
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `brilson-visiting-cards-page-${currentPage}.zip`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      setTimeout(() => URL.revokeObjectURL(url), 5000);
      ...
      if (failedCards.length > 0) {
        toast.success(`Downloaded ${successfulCards.length} visiting cards, ${failedCards.length} failed`);
      } else {
        toast.success(`All ${successfulCards.length} visiting cards downloaded successfully!`);
      }
    ...
    <div className="px-3 sm:px-4 md:px-6 lg:px-2 py-4 text-gray-200 max-w-full overflow-x-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between gap-4 mb-6 lg:mt-0 mt-11">
        <div className="w-full lg:w-auto text-center lg:text-left">
          <h4 className="text-xl font-bold">Manage Cards</h4>
          <p className="text-gray-400 mt-1 text-xs">
            View, track and manage all card profiles
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto items-stretch sm:items-center">
          ...
          >
            {downloading ? (
              ...
              <>
                ...
                <span>NFC Cards ({cards.length})</span>
              </>
            )}
          </button>

          {/* Visiting Bulk Download Button */}
          <button
          ...
          >
            {downloading ? (
              ...
              <>
                <FiDownloadCloud size={16} />
                <span>Visiting Cards ({cards.length})</span>
              </>
            )}
          </button>

          {/* Color Picker - ONLY for NFC Card */}
          <button
            onClick={() => setShowColorPicker(!showColorPicker)}
            className="px-4 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-lg flex items-center justify-center gap-2 text-white hover:shadow-lg transition-all cursor-pointer text-sm sm:text-base"
          >
            ...
            <span className="hidden xs:inline">Customize NFC</span>
      ...
      {/* COLOR PICKER MODAL - ONLY for NFC Card */}
      {showColorPicker && (
                  ...
                  <td className="p-3 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      ...
                      <button
                        ...
                        disabled={!card.qrUrl}
                        className="bg-purple-500 hover:bg-purple-600 p-1.5 rounded-lg text-white transition-all"
                  ...
                  <td className="p-3 text-center">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        ...
                        className={`group relative flex items-center flex-col gap-2 px-3 py-1.5 rounded-lg transition-all duration-300 ease-in-out cursor-pointer font-Roboto ${copyLinkId === card?.activationCode ? 'bg-green-500/20 text-green-400 ring-1 ring-green-500/50' : 'bg-gray-800/50 hover:bg-gray-700/50 text-gray-300 hover:text-white'} ${card?.indicater === "green" ? 'hover:ring-1 hover:ring-green-500/30' : ''} ${card?.indicater === "yellow" ? 'hover:ring-1 hover:ring-yellow-500/30' : ''} ${card?.indicater === "red" ? 'hover:ring-1 hover:ring-red-500/30' : ''} hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed`}
                        disabled={!card?.activationCode}
                        title="Copy profile link"
                      >
                        <div className="flex items-center gap-2">
                          <svg className={`w-3.5 h-3.5 transition-transform duration-300 ${copyLinkId === card?.activationCode ? 'scale-110' : 'group-hover:scale-110'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <span className={`ml-0.5 px-2 py-0.5 rounded-full text-xs font-bold transition-all duration-300 ${card?.indicater === "green" ? 'bg-green-500/20 text-green-400 ring-1 ring-green-500/30' : ''} ${card?.indicater === "yellow" ? 'bg-yellow-500/20 text-yellow-400 ring-1 ring-yellow-500/30' : ''} ${card?.indicater === "red" ? 'bg-red-500/20 text-red-400 ring-1 ring-red-500/30' : ''} ${!card?.indicater ? 'bg-gray-700/50 text-gray-400 ring-1 ring-gray-600/30' : ''} ${copyLinkId === card?.activationCode ? 'animate-pulse' : ''}`}>
                            {card?.copyCount || 0}
                          </span>
