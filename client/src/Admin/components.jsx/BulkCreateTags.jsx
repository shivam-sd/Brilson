...
const BulkCreateTags = () => {
  ...
  const handleCreate = async () => {
    if (count < 1 || count > 1000) {
      toast.error("Tag count must be between 1 and 1000");
      return;
    }
    ...
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/api/tags/bulk`,
        { count },
        { withCredentials: true } // ✅ cookie based auth
      );

      toast.success(res.data.message || "Tags generated successfully!", {
        position: "top-center",
      });

      setTimeout(() => {
        navigate("/admindashboard/manage-parking-tag");
      }, 1200);
    } catch (err) {
      toast.error(
    ...
    <div className="w-full min-h-screen fixed bg-gradient-to-bl from-slate-800 via-slate-900 to-slate-800 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-white mb-2">
          Bulk Parking Tag Generator
        </h2>
        <p className="text-gray-400 text-sm mb-6">
          Generate multiple NFC tags in one click
        </p>
